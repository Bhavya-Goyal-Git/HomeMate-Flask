<template>
  <Teleport to="body">
    <ModalComp @destroymodal="killReview">
      <h1 class="page-head">Review Service Professional</h1>
      ...
      <div class="input-container">
        <textarea
          ...
          v-model="review"
        />
        <label for="input-field" class="input-label">Review</label>
        <span class="input-highlight"></span>
      </div>
      <button class="modal-re-btn" @click="subReview" style="margin-left: 25vw">
        Send Review
      </button>
    </ModalComp>
  </Teleport>
...
function killReview() {
  emit("closereview", false);
}
function subReview() {
  if (!review.value || !stars.value) {
    notifStore.addNotif("error", "Review failed!", "Input fields empty!");
  }
  let payload = {
    stars: stars.value,
    review: review.value,
  };
  backend_req(`/professional/review/${props.pid}`, "POST", payload).then(
    (val) => {
      if (val) {
        notifStore.addNotif(
          "success",
          "Review Submitted",
          "Professional review done successfully!"
        );
